...
class FatEditorScreen extends ConsumerStatefulWidget {
  const FatEditorScreen({super.key, this.entryId, this.initialTargetAt});

  final String? entryId; // if present → edit mode
  final DateTime? initialTargetAt; // prefill for create

  @override
  ConsumerState<FatEditorScreen> createState() => _FatEditorScreenState();
}

class _FatEditorScreenState extends ConsumerState<FatEditorScreen> {
  ...
  Future<void> _save(BuildContext context) async {
    ...
    try {
      if (widget.entryId != null) {
        ...
        if (context.mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Updated Fat entry')),
          );
        ...
        if (context.mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Saved Fat entry')),
          );
          Navigator.of(context).pop();
        }
      }
