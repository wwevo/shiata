...
- **Handedness Feature**: Completely removed handedness-dependent layouts and controls.
  ...
  * Simplified `MiddlePanel` and `DayDetailsPanel` by removing conditional padding and alignment logic.
- Removed autogenerated migration code...
- Removed the legacy payload['recipe_id'] fallback in recipe_instance_dialog.dart
- Removed Calendar and Overview pages from the UI, using the new 7 day seven week window page
...
- **Build Artifacts**: Cleaned up generated Linux plugin files and updated `.gitignore`.
- **Build System**: Upgraded Gradle wrapper (`8.14`), Android Gradle Plugin (`8.11.1`), Kotlin plugin (`2.2.20`), and updated Android `compileSdk` to 37.
